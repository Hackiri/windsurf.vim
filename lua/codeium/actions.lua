...
function ActionContext:get_selected_text()
	...
	if self.range then
		local lines = vim.api.nvim_buf_get_lines(self.bufnr, self.range.start.line, self.range["end"].line + 1, false)
...
function ActionContext:get_surrounding_context(lines_before, lines_after)
	...
	local end_line = math.min(vim.api.nvim_buf_line_count(self.bufnr) - 1, current_line + lines_after)
...
function Action:new(config)
	local instance = {
		...
		transform = config.transform or function(ctx)
			return ctx
		end,
		post_process = config.post_process or function(result)
			return result
		end,
