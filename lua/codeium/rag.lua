...
function M.start_service()
	...
	local docker_cmd = {
		...
		"-d",
		"--name",
		M.config.container_name,
		"-p",
		string.format("%d:8080", M.config.port),
		"-v",
		string.format("%s:/workspace:ro", M.config.host_mount),
		"-e",
		string.format("LLM_PROVIDER=%s", M.config.llm.provider),
		"-e",
		string.format("LLM_ENDPOINT=%s", M.config.llm.endpoint),
		"-e",
		string.format("LLM_API_KEY=%s", vim.env[M.config.llm.api_key] or ""),
		"-e",
		string.format("LLM_MODEL=%s", M.config.llm.model),
		"-e",
		string.format("EMBED_PROVIDER=%s", M.config.embed.provider),
		"-e",
		string.format("EMBED_ENDPOINT=%s", M.config.embed.endpoint),
		"-e",
		string.format("EMBED_API_KEY=%s", vim.env[M.config.embed.api_key] or ""),
		"-e",
		string.format("EMBED_MODEL=%s", M.config.embed.model),
	}
...
function M.query_context(query, files, callback)
	if not M.is_available() then
		if callback then
			callback(nil, "RAG service is not available")
		end
		return
	end
	...
	if not M.is_service_running() then
		...
		if not M.start_service() then
			if callback then
				callback(nil, "Failed to start RAG service")
			end
			return
		end
	...
	curl.post(endpoint, {
		...
		callback = function(response)
			...
			if not ok then
				if callback then
					callback(nil, "Failed to parse RAG response")
				end
				return
			end
...
function M.index_files(files, callback)
	if not M.is_available() then
		if callback then
			callback(false, "RAG service is not available")
		end
		return
	end
	...
	if not M.is_service_running() then
		if callback then
			callback(false, "RAG service is not running")
		end
		return
	end
