...
function ActionRegistry:get_performance_report()
    ...
    table.sort(sorted_actions, function(a, b) return a.avg_time < b.avg_time end)
...
function ActionRegistry:__tostring()
    return string.format("ActionRegistry[%d actions, %d executions]", 
        vim.tbl_count(self.actions), #self.execution_history)
end
...
local function create_stateful_action_factory()
    ...
    function() return factory_state end
end
...
function LuaActionError:__tostring()
    return string.format("[%s] %s (at %s)", 
        self.error_type, self.message, os.date("%H:%M:%S", self.timestamp))
end
...
local function safe_execute_action(action_name, context, options)
    ...
    local function execute_with_recovery()
        ...
        if success then
            ...
            local error_obj = LuaActionError:new(
                result, 
                context, 
                "execution_error"
            )
...
Provide the complete metatable-enhanced version.
        ...
        validate = function(context)
            return context.filetype == "lua" and 
                   context:get_selected_text():match("function") and
                   not context:get_selected_text():match("setmetatable"),
    ...
    vim.api.nvim_create_user_command("WindsurfRegistryStats", function()
        ...
        for category, stats in pairs(report.categories) do
            table.insert(lines, string.format("%s: %d actions, %d executions, %.3fs avg", 
                category, stats.count, stats.total_executions, stats.average_time))
        end
