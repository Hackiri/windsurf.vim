...
function M.switch_provider(provider_name)
  ...
  if provider.api_key_env then
    ...
    if not api_key then
      notify.error(string.format("API key not found for %s. Please set %s or AVANTE_%s environment variable.", 
        provider.name, provider.api_key_env, provider.api_key_env))
      return false
    end
  end
...
function M.show_provider_menu()
  ...
  for _, provider in ipairs(providers) do
    ...
    local features = {}
    if provider.supports_chat then table.insert(features, "chat") end
    if provider.supports_completion then table.insert(features, "completion") end
    if provider.supports_fast_apply then table.insert(features, "fast-apply") end
    
    table.insert(items, string.format("%s%s - %s", 
      provider.display_name, status, table.concat(features, ", ")))
  end
