...
function M.execute_glob(pattern, args)
	...
	local cmd = string.format("find %s -name %s -type f", vim.fn.shellescape(workspace_root), vim.fn.shellescape(pattern))
...
function M.execute_search_keyword(keyword, args)
	...
	local workspace_root = args.cwd or util.get_project_root()
	local cmd = string.format("grep -r %s %s", vim.fn.shellescape(keyword), vim.fn.shellescape(workspace_root))
