...
function M.get_codebase_context()
  ...
  local files = {}
  local extensions = {".lua", ".py", ".js", ".ts", ".go", ".rs", ".c", ".cpp", ".h", ".hpp", ".java", ".kt", ".swift", ".rb", ".php", ".cs", ".scala", ".clj", ".hs", ".ml", ".elm", ".dart", ".r", ".jl"}
...
Cursor position: Line %d, Column %d
]], ctx.relative_path, ctx.filetype, ctx.content, ctx.cursor_pos[1], ctx.cursor_pos[2]))
    ...
    elseif ctx.type == "diagnostics" then
      ...
      for _, diag in ipairs(ctx.diagnostics) do
        table.insert(formatted, string.format("- Line %d:%d [%s] %s", 
          diag.line, diag.column, diag.severity, diag.message))
      end
    ...
    elseif ctx.type == "quickfix" then
      ...
      for _, item in ipairs(ctx.items) do
        ...
        table.insert(formatted, string.format("- %s:%d:%d %s", 
          filename, item.line, item.column, item.text))
      end
    ...
    elseif ctx.type == "buffers" then
      ...
      for _, buf in ipairs(ctx.buffers) do
        ...
        table.insert(formatted, string.format("- %s (%s, %d lines)%s", 
          buf.relative_path, buf.filetype, buf.line_count, status))
      end
    end
