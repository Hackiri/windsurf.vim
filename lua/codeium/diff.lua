...
function M.apply_code_block(code_block, options)
	...
	notify.info(string.format("Applied code changes to %s (%s mode)", vim.fn.fnamemodify(file_path, ":t"), mode))
...
function M.apply_with_confirmation(response, context)
	...
	if #code_blocks == 1 then
		...
		for i, block in ipairs(code_blocks) do
			...
			table.insert(items, string.format("%d. %s (%s) - %s", i, file_path or "unknown", block.language, preview))
		end
